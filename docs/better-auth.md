...
3. Declare `BETTER_AUTH_SECRET` and `SITE_URL` in the Convex app definition so
...
13. Keep `/login` and `/signup` outside the sidebar and redirect authenticated
    visitors back to `/`.
...
16. Set independent development, preview-default, and production Convex
    secrets and site URL fallbacks.
...
| `NEXT_PUBLIC_CONVEX_SITE_URL` | Next.js/Vercel               | Required matching `.convex.site` URL for the auth proxy. The Vercel build receives it from `convex deploy`.                             |
...
| `CONVEX_SITE_URL`             | Convex runtime/build command | Platform-provided. Do not configure it manually.                                                                                        |
...
4. Confirm `POST /api/auth/sign-up/email` returns `200`, the session endpoint
   returns the same user, and `/` renders the user's name and email in the
   sidebar.
...
This slice intentionally follows the component's starter setting
`requireEmailVerification: false`. Login, signup, session restoration, protected
routing, current-user display, and logout are covered.

The PRD's password-reset and verified-email requirements still need an email
provider, send callbacks, and reset/verification pages. They are not represented
as complete by this setup.
