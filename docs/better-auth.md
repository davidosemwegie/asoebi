...
| `EMAIL_FROM`                  | Every Convex deployment               | Sender in `Name <address@example.com>` form. A verified Resend domain is required for live delivery. Test mode may use `Aso Circle <onboarding@resend.dev>`.                                       |
...
| `NEXT_PUBLIC_SITE_URL`        | Next.js/Vercel                        | Required public origin for metadata and future absolute verification/reset links. Local development uses `http://localhost:3000`; Vercel can derive a safe fallback from its system URL variables. |
...
printf '%s\n' 'Aso Circle <onboarding@resend.dev>' | pnpm --filter @workspace/backend exec convex env set EMAIL_FROM
